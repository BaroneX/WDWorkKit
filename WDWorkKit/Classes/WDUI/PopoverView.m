...
-(CGRect)getViewFrame
{
    ...
    frame.size.height = [self.titleArray count] * ROW_HEIGHT + SPACE;
    
    frame.origin.x = WinSize.width*3/5;
    frame.origin.y = self.showPoint.y;
    frame.size.width = WinSize.width*2/5;
//    for (NSString *title in self.titleArray) {
//        CGFloat width =  [title sizeWithFont:TITLE_FONT constrainedToSize:CGSizeMake(1000, 100) lineBreakMode:NSLineBreakByCharWrapping].width;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell == nil) {
        ...
        UIImageView* imageView = [[UIImageView alloc]initWithFrame:CGRectMake(10, 5, ROW_HEIGHT-10, ROW_HEIGHT-10)];
        imageView.tag = 1;
        [cell.contentView addSubview:imageView];
        
        
        UILabel* label = [[UILabel alloc]initWithFrame:CGRectMake(imageView.right+5, imageView.top, tableView.width-imageView.right-10, imageView.height)];
...
//    cell.backgroundView.backgroundColor = RGB(245, 245, 245);
    cell.backgroundColor = RGB(48, 48, 48);
